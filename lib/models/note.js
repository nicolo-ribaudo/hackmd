    ...
    classMethods: {
      ...
      parseNoteInfo: function (body) {
        ...
        return {
          ...
          category: Note.extractNoteCategories(parsed.meta, $),
          categories: Note.extractNoteCategories(parsed.meta, $)
        }
      },
      ...
      extractNoteCategories: function (meta, $) {
        var categories = ''
        if (meta.categories && (Object.prototype.toString.call(meta.categories) === '[object Array]' )) {
      ...
      extractNoteCategories: function (meta, $) {
        var categories = []
        var rawcategories = []
        if (meta.categories && (typeof meta.categories === 'string' || typeof meta.categories === 'number')) {
          var metacategories = ('' + meta.categories).split(',')
          for (let i = 0; i < metacategories.length; i++) {
            var text = metacategories[i].trim()
            if (text) rawcategories.push(text)
          }
        }
        for (let i = 0; i < rawcategories.length; i++) {
          var found = false
          for (let j = 0; j < categories.length; j++) {
            if (categories[j] === rawcategories[i]) {
              found = true
              break
            }
          }
          if (!found) { categories.push(rawcategories[i]) }
        }
        return categories
      },
    ...
    hooks: {
      beforeCreate: function (note, options, callback) {
        ...
        if (!note.content) {
          var body = null
          let filePath = null
          filePath = path.join(config.docspath, note.id + '.md')
          if (! Note.checkFileExist(filePath)) {
            if (options.template != null) {
              filePath = path.join(config.publicpath, options.template + '.md')
      ...
      afterUpdate: function (note, options, callback) {
        logger.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.")
        logger.info(note.lastChangedAt)
        logger.info(note.savedAt)
      },      
    }
  })
